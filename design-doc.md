...
**Parent-child chunking** (v2, **implemented V-49**): a parent vector stores a document summary with `memory_type: "document"`; child vectors store chunks with `parent_key` metadata pointing at the parent. Retrieval promotes chunk hits to the parent summary; agents fetch child chunks on demand via `list_memories({parent_key})` or `expand_cites`. Existing single-vector memories are unchanged — re-ingest long docs as document+chunks when finer retrieval is needed.
...
| **3. Context budget** | Collapsed candidates | Top 5 (final list) | Rank by query distance (or metadata-aware re-score when `rank_mode=balanced|procedural` — V-45); trim to `max_tokens` (default 4,000 tokens of memory content). Prefer `content_summary` when budget is tight; fetch full `content` / S3 only when `detail_level` requests hydration (V-44). |

**Metadata ranking (V-45)**: After collapse, `rank_mode` selects ordering before the context budget: `semantic` preserves pure cosine distance (legacy); `balanced` (default) applies lightweight metadata boosts (memory_type, episodic recency decay, writer confidence) plus MMR diversity across `canonical_id`/`task_id` — zero additional AWS API calls; `procedural` strongly prefers `memory_type=procedural`. Emits `RetrieveRankMode` CloudWatch metric.

**Usage feedback (popularity tiebreaker, V-53)**: Hydration — deliberately expanding a memory to its full body via `hydrate_memory`, `hydrate_keys`, or STANDARD/full-detail auto-hydration — is the implicit usage signal: it increments `use_count` and stamps `last_used_at` on the canonical row. Retrieval that returns only a `content_summary` does **not** count as use, so summary-only browsing can't create a rich-get-richer loop. In `ranking._base_score`, popularity contributes a small, recency-decayed boost (`POPULARITY_WEIGHT * log1p(use_count) * decay`, decay halving every `POPULARITY_HALFLIFE_DAYS` (30) days) capped at `POPULARITY_MAX` (0.04) — negligible next to relevance scores (~1.0), so it only breaks near-ties and never overrides a real relevance gap. Applies to `rank_mode=balanced|procedural` (V-45); `semantic` ignores it. Counters live only in the best-effort DynamoDB `memory-index` canonical rows (`canonical_index.py`); they never touch vector metadata and are never a correctness dependency — retrieval and ranking degrade gracefully if counters are absent or stale. `reinforce_memory` is an optional, explicit "this was useful" verb that bumps the same counter; it is mutating and excluded from the read-only auditor role's tool set, and is never required for correct retrieval.
...
> **Forward note — Rerank 4 Fast supersedes Rerank 3.5.** Cohere Rerank 4 Fast is the successor to Rerank 3.5. Amazon Bedrock does not host a Rerank 4 model yet (the Bedrock reranking table lists only `amazon.rerank-v1:0` and `cohere.rerank-v3-5:0`), and VectorVault reranks keyless via Bedrock IAM. Migrate `RERANK_MODEL_ID` (`rerank.py`) and the IAM model ARN (`infra/lib/memory-stack.ts`) to the Rerank 4 Fast model ID once Bedrock hosts it; re-verify the query price at that time.
...
**Agent identity naming convention (v1.8, normative).** Because `agent_id` is convention-enforced, ids MUST be unambiguous by construction: interactive agent sessions use `<agent>-<project-slug>` (e.g. `claude-vv`, `grok-siege`) — one session, one project, one id, never reused across projects; utility processes use `<purpose>-bot`; test/probe processes use `e2e-*`/`*-probe` and never write durable memories. Slugs are registered in the vault's live agent directory (team `vectorvault`, task `agent-directory`). Full rules + legacy-id mapping: the CLI-agents runbook §"Agent identity convention".
...
| 1.6 | Deployment Region set (owner, 2026-07-08) | Deploy Region fixed to **us-west-2** (owner's `<your-profile>` profile default); cost table relabeled; pricing unchanged vs us-east-1. Region-bound resources (KMS, buckets) are one-way doors. |
